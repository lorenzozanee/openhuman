...
//!
//! [`crate::memory::api`] is the host-owned memory contract: `MemoryProvider`,
...
/// `endpoint` from [`crate::config::schema::MemoryDriverConfig`],
...
fn module_provider(
    ...
    (
        Arc::new(
            crate::modules::memory::ModuleMemoryProvider::from_boot_policy()
...
/// see [`crate::memory::test_support::settle_memory_module`].
#[cfg(all(feature = "modules", test))]
pub(crate) fn test_module_config() -> crate::config::Config {
    let workspace_dir = crate::memory::ops::shared_memory_test_workspace();
...
fn module_provider(
    ...
    (
        Arc::new(
            crate::modules::memory::ModuleMemoryProvider::new(Arc::new(test_module_config()))
                .in_subdir(memory_subdir),
        ),
        DriverClass::Module,
    )
